...
exports.RETRY_CODES = [
    grpc_1.status.OK,
    grpc_1.status.CANCELLED,
    grpc_1.status.UNKNOWN,
    grpc_1.status.DEADLINE_EXCEEDED,
    grpc_1.status.RESOURCE_EXHAUSTED,
    grpc_1.status.ABORTED,
    grpc_1.status.INTERNAL,
    grpc_1.status.UNAVAILABLE,
    grpc_1.status.DATA_LOSS,
];
/**
...
class PullRetry {
     ...
     */
    retry(err) {
        if (err.code === grpc_1.status.OK || err.code === grpc_1.status.DEADLINE_EXCEEDED) {
            this.failures = 0;
        }
        else {
            this.failures += 1;
